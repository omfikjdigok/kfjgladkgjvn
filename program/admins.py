...
@Client.on_message(command(["تحديث","/reload", f"/reload@{BOT_USERNAME}"]) & other_filters)
...
@Client.on_message(command(["تخطي","/skip", f"/skip@{BOT_USERNAME}", "/vskip"]) & other_filters)
@authorized_users_only
    ...
    keyboard = InlineKeyboardMarkup(
        [
            [
                ...
                [
            InlineKeyboardButton( 
                        "- قناة البـوت", url="https://t.me/nayy2019" 
                    ),
            ] 
        ]       
...
@Client.on_message(
    command(["مريم انزلي","ايقاف الكل","/stop", f"/stop@{BOT_USERNAME}", "/end", f"/end@{BOT_USERNAME}", "/vstop"])
    & other_filters
)
@authorized_users_only
...
@Client.on_message(
    command(["ايقاف موقت","/pause", f"/pause@{BOT_USERNAME}", "/vpause"]) & other_filters
)
@authorized_users_only
...
@Client.on_message(
    command(["استئناف","/resume", f"/resume@{BOT_USERNAME}", "/vresume"]) & other_filters
)
@authorized_users_only
...
@Client.on_message(
    command(["كتم البوت","/mute", f"/mute@{BOT_USERNAME}", "/vmute"]) & other_filters
)
@authorized_users_only
...
@Client.on_message(
    command(["الغاء كتم البوت","/unmute", f"/unmute@{BOT_USERNAME}", "/vunmute"]) & other_filters
)
@authorized_users_only
...
@Client.on_message(
    command(["ضبط الصوت","volume", f"volume@{BOT_USERNAME}", "vol"]) & other_filters
)
@authorized_users_only
