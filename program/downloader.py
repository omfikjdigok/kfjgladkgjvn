...
@Client.on_message(command(["بحث","/song", f"/song@{bn}"]) & ~filters.edited)
def song(_, message):
    query = " ".join(message.command[1:])
    m = message.reply("🔎 الحين ادوره واعطيك...")
    ...
    try:
        ...
        print(str(e))
        return
    m.edit("- الحين انزله لك ...")
    try:
        with yt_dlp.YoutubeDL(ydl_ops) as ydl:
            info_dict = ydl.extract_info(link, download=False)
            audio_file = ydl.prepare_filename(info_dict)
            ydl.process_info(info_dict)
        rep = f"**🎧تـم حملته لك @{bn}**"
        secmul, dur, dur_arr = 1, 0, duration.split(":")
        for i in range(len(dur_arr) - 1, -1, -1):
            dur += int(float(dur_arr[i])) * secmul
            secmul *= 60
        m.edit("📤- الحين ارسله لك ...")
        message.reply_audio(
            audio_file,
            caption=rep,
            thumb=thumb_name,
...
@Client.on_message(
    command(["بحث_فيديو","/vsong", f"/vsong@{bn}", "/video", f"/video@{bn}"]) & ~filters.edited
)
async def vsong(client, message):
    ...
    try:
        ...
        print(e)
    try:
        msg = await message.reply("📥 **الحين احملك الفيديو انتظرني**")
    ...
    await msg.edit("📤 **الحين ارسلك الفيديو شكراً على الانتظار**")
